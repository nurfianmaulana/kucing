...
def riwayat_list(request):
    if request.method == 'GET':
        gejala = models.Riwayat.objects.all()
        serializer = serializers.RiwayatSerializer(gejala, many=True)
        return Response(serializer.data)
...
def detail_riwayat(req, id):
    try:
        gejala = models.Riwayat.objects.get(pk=id)
    except models.Riwayat.DoesNotExist:
        return HttpResponse(status=status.HTTP_404_NOT_FOUND)

    if req.method == 'GET':
        serializer = serializers.RiwayatSerializer(gejala)
        return Response(serializer.data)

    elif req.method == 'PATCH':
        serializer = serializers.RiwayatSerializer(gejala, data=req.data, partial=True)
